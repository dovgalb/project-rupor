...
type ListMembersHandler struct {
	uc *usecase.ListMembers
}

func NewListMembersHandler(uc *usecase.ListMembers) *ListMembersHandler {
	return &ListMembersHandler{uc: uc}
}
...
func (h *ListMembersHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	out, err := h.uc.Execute(r.Context(), usecase.ListMembersInput{
		ActorID: uid.UUID(),
		RoomID:  roomID,
	})
	if err != nil {
	...
	items := make([]memberResponse, 0, len(out.Items))
	for _, m := range out.Items {
		items = append(items, membershipToResponse(m))
	}

	w.Header().Set("Content-Type", "application/json; charset=utf-8")
	w.WriteHeader(http.StatusOK)
	_ = jsonEncode(w, listMembersResponse{Items: items})
}
